...
const CreatePoint = () => {
  ...
  const [selectedFile, setSelectedFile] = useState<File>(); //Armazenando uma informação do tipo file

  const [openModal, setOpenModal] = useState(false);

  const history = useHistory(); //historico da sessao
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (selectedFile) {
      data.append('image', selectedFile); //não deixa dar submit sem carregar imagem
    }
  ...
  return (
    <div id="page-create-point">
      ...
      <ModalComponent
        className={!openModal ? 'hide modal-success' : 'show modal-success'}
      />
    </div>
  );
};
